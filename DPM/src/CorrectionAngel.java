	...
	public void run() {
		...
		while (true) {
			...
			if (lineFinderRight.line() && !locked) {
				...
				break;
				// LCD.drawString("Final "+ Math.toDegrees(odo.getTheta()), 4,
				// 6);
				// Sound.twoBeeps() ;
			...
			// *
			/*
			 * updateEnd = System.currentTimeMillis() ; if (updateEnd -
			 * updateStart < PERIOD) { try { Thread.sleep(PERIOD - (updateEnd -
			 * updateStart)); } catch (InterruptedException e) { // there is
			 * nothing to be done here because it is not // expected that the
			 * odometer will be interrupted by // another thread } }
			 */
